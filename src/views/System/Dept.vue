<template>
  ...
  <div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
    <el-form :inline="true" :model="filters" :size="size">
      ...
      <el-form-item>
        <hm-button icon="fa fa-search" label="查询" perms="system:dept:list" type="primary" @click="findTreeData(null)"/>
      </el-form-item>
      <el-form-item>
        <hm-button icon="fa fa-plus" label="增加" perms="system:dept:add" type="primary" @click="handleAdd"/>
      </el-form-item>
    </el-form>
  </div>
    <!--表格树内容栏-->
    <el-table :data="tableTreeDdata" stripe size="mini" style="width: 100%;"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <hm-button icon="fa fa-edit" label="编辑" perms="system:dept:update" @click="handleEdit(scope.row)"/>
          <hm-button icon="fa fa-trash" label="删除" perms="system:dept:remove" type="danger" @click="handleDelete(scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    <!-- 新增修改界面 -->
...
export default {
  ...
  methods: {
    ...
    findTreeData: function () {
      ...
      this.$api.dept.list(params).then((res) => {
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            if (this.type === 1) {
              ...
              this.$api.dept.update(this.dataForm, params).then(res => {
                this.editLoading = false
                if (res.status === 1) {
                  this.$message({ message: '操作成功', type: 'success' })
                  this.$refs['dataForm'].resetFields()
